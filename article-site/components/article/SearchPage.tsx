...
export function SearchPage({ tags, writers, currentQ, currentTag, currentWriter, articles, isFiltering }: Props) {
  ...
  return (
    <div>
      <h1 className="text-base font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">検索</h1>
      ...
      {tags.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-semibold text-gray-500 mb-2">タグでしぼる</p>
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag) => (
              <button key={tag.id} onClick={() => handleTagClick(tag.slug)}
                className={['px-3 py-1 rounded-full text-xs font-medium border transition-colors',
                  selectedTag === tag.slug ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300'].join(' ')}>
                #{tag.name}
              </button>
            ))}
          </div>
      ...
      {writers.length > 0 && (
        <div className="mb-6">
          <p className="text-xs font-semibold text-gray-500 mb-2">ライターでしぼる</p>
              ...
              return (
                <button key={w.id} onClick={() => handleWriterClick(w.id)}
                  className={['flex items-center gap-2 px-3 py-2 rounded-lg border text-left transition-colors',
                    selected ? 'bg-blue-700 text-white border-blue-700' : 'bg-white border-gray-200 hover:border-blue-300'].join(' ')}>
      ...
      {isFiltering && (
        <div>
          <h2 className="text-sm font-bold text-blue-800 border-b-2 border-blue-800 pb-1 mb-3">
            検索結果 <span className="text-gray-400 font-normal">{articles.length}件</span>
          </h2>
          {articles.length === 0 ? (
            <p className="text-center text-gray-400 py-8 text-sm">条件に一致する記事がありません</p>
          ) : (
