    ...
    <div className="max-w-2xl mx-auto px-4 py-4">
      ...
      {favorites.length === 0 && liked.length === 0 && (
        <div className="text-center py-12 text-gray-400">
          <p className="text-sm mb-1 dark:text-gray-400">まだお気に入りに登録した記事がありません</p>
          <p className="text-xs dark:text-gray-500">好きな記事をお気に入り登録しよう</p>
        </div>
      )}
      ...
      <section className="mb-8">
        <h2 className="text-sm font-bold text-blue-800 dark:text-blue-400 border-b-2 border-blue-800 dark:border-blue-400 pb-1 mb-3">
          保存した記事 <span className="text-gray-400 dark:text-gray-500 font-normal text-xs">({favorites.length})</span>
        </h2>
        {favorites.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-gray-500 py-4 text-center">まだ保存した記事がありません</p>
        ) : (
      ...
      <section>
        <h2 className="text-sm font-bold text-blue-800 dark:text-blue-400 border-b-2 border-blue-800 dark:border-blue-400 pb-1 mb-3">
          いいねした記事 <span className="text-gray-400 dark:text-gray-500 font-normal text-xs">({liked.length})</span>
        </h2>
        {liked.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-gray-500 py-4 text-center">まだいいねした記事がありません</p>
        ) : (
