...
export default async function AdminPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4 space-y-8">
      <h1 className="text-base font-bold text-gray-900 dark:text-gray-100 pb-2 border-b border-gray-200 dark:border-gray-700">管理画面</h1>
      ...
      <section>
        ...
        {!pendingArticles || pendingArticles.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 py-4 text-center bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">承認待ちの記事はありません</p>
        ) : (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm divide-y divide-gray-100 dark:divide-gray-700 border border-gray-100 dark:border-gray-700">
              ...
              return (
                <div key={article.id} className="flex items-center gap-4 px-5 py-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{authorName}</p>
                  </div>
      ...
      {/* おすすめ設定 */}
      <section>
        <h2 className="text-sm font-bold text-blue-800 dark:text-blue-400 border-b-2 border-blue-800 dark:border-blue-400 pb-1 mb-3">おすすめ記事の設定</h2>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm divide-y divide-gray-100 dark:divide-gray-700 border border-gray-100 dark:border-gray-700">
          {!publishedArticles || publishedArticles.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-500 py-4 text-center">公開済みの記事がありません</p>
              ...
              <div key={article.id} className="flex items-center gap-4 px-5 py-4">
                <div className="flex-1 min-w-0">
                  <Link href={'/articles/' + article.id}
                    className="font-medium text-gray-800 dark:text-gray-100 hover:text-blue-700 dark:hover:text-blue-400 transition-colors line-clamp-1 text-sm">
                    {article.title}
                  </Link>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{authorName}</p>
                </div>
                <FeaturedToggle articleId={article.id} initialIsFeatured={article.is_featured} />
                <Link href={'/articles/' + article.id + '/edit'} className="text-xs text-blue-600 dark:text-blue-400 hover:underline whitespace-nowrap">編集</Link>
      ...
      {/* ユーザー管理 */}
      <section>
        <h2 className="text-sm font-bold text-blue-800 dark:text-blue-400 border-b-2 border-blue-800 dark:border-blue-400 pb-1 mb-3">ユーザー管理</h2>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm divide-y divide-gray-100 dark:divide-gray-700 border border-gray-100 dark:border-gray-700">
          {(profiles ?? []).map((p) => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-4 px-5 py-4">
                <div className="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden flex-shrink-0 flex items-center justify-center">
                  {p.avatar_url ? (
                    <Image src={p.avatar_url} alt={displayName} width={36} height={36} className="object-cover w-full h-full" />
                  ) : (
                    <span className="text-gray-500 dark:text-gray-400 text-sm font-bold">{displayName[0].toUpperCase()}</span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                ...
                {p.id !== user.id ? (
                  ...
                  <span className="text-xs text-gray-400 dark:text-gray-500 italic">（自分）</span>
                )}
              </div>
            );
          })}
